...
list<shared_ptr<Fellow> > FeiqModel::searchFellow(const string &text) const
{
    lock_guard<mutex> guard(mFellowLock);
...
shared_ptr<FileTask> FeiqModel::findTask(IdType packetNo, IdType fileId, FileTaskType type) const
{
    lock_guard<mutex> g(mFileTaskLock);
...
list<shared_ptr<FileTask> > FeiqModel::searchTask(function<bool (const FileTask &)> predict) const
{
    lock_guard<mutex> g(mFileTaskLock);
